...
int main(int argc, char **argv)
{
    if(argc != 2)
    {
        std::cerr << std::endl << "Usage: ./bag_creator_node config_file_path" << std::endl;
    ...
    // bag.open("/home/Rossen/Datasets/MH_01_easy.bag", rosbag::bagmode::Read);
    ...
    vTimeStamps.reserve(72000);// enough for up to 30min@40Hz sequence

    int down_sample_ratio = config_node["down_sample_ratio"].as<int>();// sometimes you may downsample to cut the bag size
    ...
    const int nTimes = vTimeStamps.size();// number of frames
    ...
    for (size_t i = 0; i < nTimes; i++)
    {
        ...
        // cv::Mat img = cv::imread(l_path + file_name, cv::IMREAD_GRAYSCALE);// gray images option TBD
        ...
        // cv::Mat img2 = cv::imread(l_path + file_name2, cv::IMREAD_GRAYSCALE);
        cv::Mat img2 = cv::imread(r_path + file_name2, imread_flag);// gray images
        if (img2.empty()){
